...
import { Bot, X, Send, User, Sparkles, RefreshCw, MessageSquare } from 'lucide-react';
...
const SUGGESTED_PROMPTS = [
  "What experience does Shahid have with SQL & databases?",
  "Tell me about Shahid's Super Store EDA project.",
  "What Power BI & DAX skills does Shahid possess?",
  "What is Shahid's educational background and degree status?",
  "How can I contact Shahid for a Data Analyst role?"
];
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 'welcome-msg',
      sender: 'assistant',
      text: "Hello! I'm Shahid's AI Assistant. I can answer questions about his data analytics background, Python/SQL projects, Power BI dashboards, and availability. How can I help you today?",
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: (Date.now() + 1).toString(),
          sender: 'assistant',
          text: "Shahid Ahmad Sheer Gojree is a Data Analyst skilled in Python, SQL, Power BI, and Excel. You can contact him at shahidgojree880@gmail.com.",
          timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
        }
      ]);
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-slate-900/60 backdrop-blur-xs transition-opacity animate-fade-in">
      <div className="w-full max-w-lg bg-white dark:bg-slate-900 h-full flex flex-col shadow-2xl border-l border-slate-200 dark:border-slate-800">
        ...
        <div className="p-4 sm:p-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-900">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">Powered by Gemini & Portfolio Knowledge Engine</p>
            </div>
          </div>
        ...
        {/* Suggested Prompts */}
        <div className="p-3 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-x-auto">
          ...
          <div className="flex flex-wrap gap-1.5">
            {SUGGESTED_PROMPTS.map((p, idx) => (
              <button
                ...
                disabled={isLoading}
                className="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs text-left transition-colors font-medium"
              >
                {p}
              </button>
            ))}
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
          ...
          >
            <input
              ...
              placeholder="Ask about Shahid's skills, projects, or education..."
