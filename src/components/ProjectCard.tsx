    ...
    <div id={`project-${project.id}`} className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-md shadow-slate-200/50 dark:shadow-none overflow-hidden transition-all duration-300 hover:border-sky-500/50">
      ...
      <div className="p-6 md:p-8 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          ...
          {project.impactBadge && (
            <div className="self-start inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/70 border border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300 text-xs font-bold shadow-xs shrink-0">
        ...
        {/* Prominent Technology Tags */}
        <div className="flex flex-wrap items-center gap-2 mt-4">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 flex items-center gap-1 mr-1">
          ...
          {project.techTags ? (
            project.techTags.map((tag, idx) => {
              const tagStyles = {
                core: 'bg-indigo-50 dark:bg-indigo-950/70 text-indigo-700 dark:text-indigo-300 border-indigo-200 dark:border-indigo-800/80',
                framework: 'bg-sky-50 dark:bg-sky-950/70 text-sky-700 dark:text-sky-300 border-sky-200 dark:border-sky-800/80',
                database: 'bg-amber-50 dark:bg-amber-950/70 text-amber-800 dark:text-amber-300 border-amber-200 dark:border-amber-800/80',
                methodology: 'bg-purple-50 dark:bg-purple-950/70 text-purple-700 dark:text-purple-300 border-purple-200 dark:border-purple-800/80',
              }[tag.type] || 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700';

              return (
                <span
                  key={idx}
                  className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold border transition-colors ${tagStyles}`}
                >
                  {tag.type === 'database' && <Database className="w-3 h-3 opacity-70" />}
                  {tag.type === 'core' && <Code className="w-3 h-3 opacity-70" />}
                  {tag.name}
                </span>
              );
            })
            ...
            project.tools.map((t, idx) => (
              <span key={idx} className="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-medium border border-slate-200/80 dark:border-slate-700/80">
                {t}
              </span>
            ))
          )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5 mt-5">
          {project.metrics.map((m, idx) => (
            <div 
              key={idx} 
              className="p-3.5 rounded-xl bg-white dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/80 shadow-xs flex flex-col justify-between transition-all hover:border-sky-500/40"
            >
              <div className="flex items-center justify-between gap-1">
                <span className="text-[11px] uppercase tracking-wider text-slate-500 dark:text-slate-400 font-bold truncate">
                  {m.label}
                </span>
                {m.change && (
                  <span className={`inline-flex items-center text-[10px] font-bold px-1.5 py-0.5 rounded-md ${
                    ...
                    {m.trend === 'up' && <TrendingUp className="w-2.5 h-2.5 mr-0.5" />}
                    {m.trend === 'down' && <TrendingDown className="w-2.5 h-2.5 mr-0.5" />}
                    {m.change}
                  </span>
                )}
              </div>

              <div className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white mt-1.5 tracking-tight">
                {m.value}
              </div>

              {m.subtext && (
                <div className="text-[10px] text-slate-500 dark:text-slate-400 font-mono mt-1 flex items-center gap-1 truncate">
                  <Activity className="w-2.5 h-2.5 text-sky-500 shrink-0" />
                  <span>{m.subtext}</span>
                </div>
              )}
            </div>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie
                          ...
                          cy="50%"
                          labelLine={true}
                          label={(entry: any) => `${entry.name} (${entry.value ?? entry.gmv}%)`}
                          outerRadius={85}
                          fill="#8884d8"
                          dataKey="gmv"
                        >
